 ...
 * 
 * @author sstein
 * @author matthias (refactorings / code fixes)
 * 
 */
...
//	private int maxPoints = 50;	//used for segmentation, which is necessary to avoid too big matrices
	...
	private LineString smoothLineMaxDisp(LineString line, double maxDisp, double alpha, double beta){
		LineString inLine = (LineString)line.clone();
		...
		while(proceed== true){
			...
			//            System.out.println("SnakesSmoothingLineNew.solveMaxDisp - Loop: " + j + " maxDisp = " + dist + " treshold = " + maxDisp);               
			if(dist > maxDisp){
				u=u+1;
				if (j == 1){
					...
					line = (LineString)inLine.clone(); //take old line
					mySnake.initialise(line,alpha,beta);
					j = 0;   
					if (u==10){
						proceed = false; 
